...
class ExternalProcess(object):
  ...
  def __getattr__(self, name):  # when getting a non-existing attribute
    ...
    Returns:
      Value of the attribute.
    """
    if ExternalProcess._process.is_alive():
  ...
  # def _worker_(self, constructoe, conn):
  #   p = multiprocessing.Process(target=self._worker, args=(constructoe, conn))
  #   p.start()
  #
  #   # self._worker(constructoe, conn)
  #
